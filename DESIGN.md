...
в его копии под `%LOCALAPPDATA%` правятся шесть байт (`internal/hlpatch`): два выключают
фатальный таймаут клиента, четыре пускают коды от 5 до 32 символов через «join by code».
