...
pub(crate) fn write_strings(w: &mut impl Write, strings: &[Str]) -> Result<()> {
    // Strings may contain embedded NUL bytes (the reader uses explicit
    // lengths), so write raw bytes + terminator instead of CString.
    let size: usize = strings.iter().map(|s| s.as_bytes().len() + 1).sum();
